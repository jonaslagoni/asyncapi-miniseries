...
describe('game/server/{serverId}/events/player/{playerId}/chat can talk to itself', () => {
  ...
  it('can send message', async () => {
    ...
    var receivedMsg: TestClient.PlayerUsedChatPayload | undefined = undefined;
    var receivedServerId: string | undefined = undefinedvar receivedPlayerId: string | undefined = undefined
    var publishMessage: Client.PlayerUsedChatPayload = {
      "chatTimestamp": "2019-08-24T14:15:22Z",
      "message": "string"
    };
    var ServerIdToSend: string = "string"
    var PlayerIdToSend: string = "string"
    const subscription = await testClient.subscribeToGameServerServerIdEventsPlayerPlayerIdChat((err, msg, serverId, playerId) => {
        receivedError = err;
        receivedMsg = msg;
    ...
    await client.publishToGameServerServerIdEventsPlayerPlayerIdChat(publishMessage, ServerIdToSend, PlayerIdToSend);
