...
describe('game/server/{serverId}/events/player/{playerId}/connect can talk to itself', () => {
  ...
  it('can send message', async () => {
    ...
    var receivedMsg: TestClient.PlayerConnectedPayload | undefined = undefined;
    var receivedServerId: string | undefined = undefinedvar receivedPlayerId: string | undefined = undefined
    var publishMessage: Client.PlayerConnectedPayload = {
      "connectTimestamp": "2019-08-24T14:15:22Z"
    };
    var ServerIdToSend: string = "string"
    var PlayerIdToSend: string = "string"
    const subscription = await testClient.subscribeToGameServerServerIdEventsPlayerPlayerIdConnect((err, msg, serverId, playerId) => {
        receivedError = err;
        receivedMsg = msg;
    ...
    await client.publishToGameServerServerIdEventsPlayerPlayerIdConnect(publishMessage, ServerIdToSend, PlayerIdToSend);
