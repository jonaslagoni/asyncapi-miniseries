...
export class NatsAsyncApiTestClient extends events.EventEmitter {
   ...
   */
  public subscribeToGameServerServerIdEventsPlayerPlayerIdItemItemIdPickup(
    onDataCallback: (
      err ? : NatsTypescriptTemplateError,
      msg ? : PlayerItemPickupPayload, serverId ? : string, playerId ? : string, itemId ? : string) => void, serverId: string, playerId: string, itemId: string,
    flush ? : boolean,
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (nc) {
        try {
          const sub = await gameServerServerIdEventsPlayerPlayerIdItemItemIdPickupChannel.subscribe(
            onDataCallback, nc, serverId, playerId, itemId,
            options
          );
          if (flush) {
   ...
   * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
   */
  public subscribeToGameServerServerIdEventsPlayerPlayerIdConnect(
    onDataCallback: (
      err ? : NatsTypescriptTemplateError,
      msg ? : PlayerConnectedPayload, serverId ? : string, playerId ? : string) => void, serverId: string, playerId: string,
    flush ? : boolean,
    options ? : SubscriptionOptions
  ): Promise < Subscription > {
    return new Promise(async (resolve, reject) => {
      ...
      if (nc) {
        try {
          const sub = await gameServerServerIdEventsPlayerPlayerIdConnectChannel.subscribe(
            onDataCallback, nc, serverId, playerId,
            options
          );
          if (flush) {
            this.jsonClient!.flush(() => {
   ...
   * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
   */
  public subscribeToGameServerServerIdEventsPlayerPlayerIdDisconnect(
    onDataCallback: (
      err ? : NatsTypescriptTemplateError,
      msg ? : PlayerDisconnectedPayload, serverId ? : string, playerId ? : string) => void, serverId: string, playerId: string,
    flush ? : boolean,
    options ? : SubscriptionOptions
  ): Promise < Subscription > {
    return new Promise(async (resolve, reject) => {
      ...
      if (nc) {
        try {
          const sub = await gameServerServerIdEventsPlayerPlayerIdDisconnectChannel.subscribe(
            onDataCallback, nc, serverId, playerId,
            options
          );
          if (flush) {
   ...
   * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
   */
  public subscribeToGameServerServerIdEventsPlayerPlayerIdChat(
    onDataCallback: (
      err ? : NatsTypescriptTemplateError,
      msg ? : PlayerUsedChatPayload, serverId ? : string, playerId ? : string) => void, serverId: string, playerId: string,
    flush ? : boolean,
    options ? : SubscriptionOptions
  ): Promise < Subscription > {
    return new Promise(async (resolve, reject) => {
      ...
      if (nc) {
        try {
          const sub = await gameServerServerIdEventsPlayerPlayerIdChatChannel.subscribe(
            onDataCallback, nc, serverId, playerId,
            options
          );
          if (flush) {
   ...
   * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
   */
  public subscribeToGameServerServerIdEventsPlayerPlayerIdHit(
    onDataCallback: (
      err ? : NatsTypescriptTemplateError,
      msg ? : AnonymousSchema_9, serverId ? : string, playerId ? : string) => void, serverId: string, playerId: string,
    flush ? : boolean,
    options ? : SubscriptionOptions
  ): Promise < Subscription > {
    return new Promise(async (resolve, reject) => {
      ...
      if (nc) {
        try {
          const sub = await gameServerServerIdEventsPlayerPlayerIdHitChannel.subscribe(
            onDataCallback, nc, serverId, playerId,
            options
          );
          if (flush) {
