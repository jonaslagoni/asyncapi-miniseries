...
const gameServerServerIdEventsPlayerPlayerIdItemItemIdPickupChannel = __importStar(require("./testchannels/GameServerServerIdEventsPlayerPlayerIdItemItemIdPickup"));
...
const gameServerServerIdEventsPlayerPlayerIdConnectChannel = __importStar(require("./testchannels/GameServerServerIdEventsPlayerPlayerIdConnect"));
exports.gameServerServerIdEventsPlayerPlayerIdConnectChannel = gameServerServerIdEventsPlayerPlayerIdConnectChannel;
const gameServerServerIdEventsPlayerPlayerIdDisconnectChannel = __importStar(require("./testchannels/GameServerServerIdEventsPlayerPlayerIdDisconnect"));
exports.gameServerServerIdEventsPlayerPlayerIdDisconnectChannel = gameServerServerIdEventsPlayerPlayerIdDisconnectChannel;
const gameServerServerIdEventsPlayerPlayerIdChatChannel = __importStar(require("./testchannels/GameServerServerIdEventsPlayerPlayerIdChat"));
exports.gameServerServerIdEventsPlayerPlayerIdChatChannel = gameServerServerIdEventsPlayerPlayerIdChatChannel;
const gameServerServerIdEventsPlayerPlayerIdHitChannel = __importStar(require("./testchannels/GameServerServerIdEventsPlayerPlayerIdHit"));
...
class NatsAsyncApiTestClient extends events.EventEmitter {
     ...
     */
    subscribeToGameServerServerIdEventsPlayerPlayerIdItemItemIdPickup(onDataCallback, serverId, playerId, itemId, flush, options) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const nc = this.jsonClient;
            if (nc) {
                try {
                    const sub = yield gameServerServerIdEventsPlayerPlayerIdItemItemIdPickupChannel.subscribe(onDataCallback, nc, serverId, playerId, itemId, options);
                    if (flush) {
     ...
     * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
     */
    subscribeToGameServerServerIdEventsPlayerPlayerIdConnect(onDataCallback, serverId, playerId, flush, options) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const nc = this.jsonClient;
            if (nc) {
                try {
                    const sub = yield gameServerServerIdEventsPlayerPlayerIdConnectChannel.subscribe(onDataCallback, nc, serverId, playerId, options);
                    if (flush) {
                        this.jsonClient.flush(() => {
                            resolve(sub);
                        });
     ...
     * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
     */
    subscribeToGameServerServerIdEventsPlayerPlayerIdDisconnect(onDataCallback, serverId, playerId, flush, options) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const nc = this.jsonClient;
            if (nc) {
                try {
                    const sub = yield gameServerServerIdEventsPlayerPlayerIdDisconnectChannel.subscribe(onDataCallback, nc, serverId, playerId, options);
                    if (flush) {
                        this.jsonClient.flush(() => {
                            resolve(sub);
                        });
     ...
     * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
     */
    subscribeToGameServerServerIdEventsPlayerPlayerIdChat(onDataCallback, serverId, playerId, flush, options) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const nc = this.jsonClient;
            if (nc) {
                try {
                    const sub = yield gameServerServerIdEventsPlayerPlayerIdChatChannel.subscribe(onDataCallback, nc, serverId, playerId, options);
                    if (flush) {
                        this.jsonClient.flush(() => {
                            resolve(sub);
                        });
     ...
     * @param options to subscribe with, bindings from the AsyncAPI document overwrite these if specified
     */
    subscribeToGameServerServerIdEventsPlayerPlayerIdHit(onDataCallback, serverId, playerId, flush, options) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const nc = this.jsonClient;
            if (nc) {
                try {
                    const sub = yield gameServerServerIdEventsPlayerPlayerIdHitChannel.subscribe(onDataCallback, nc, serverId, playerId, options);
                    if (flush) {
                        this.jsonClient.flush(() => {
                            resolve(sub);
                        });
