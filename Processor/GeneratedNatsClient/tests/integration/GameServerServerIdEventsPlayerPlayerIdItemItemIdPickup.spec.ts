...
describe('game/server/{serverId}/events/player/{playerId}/item/{itemId}/pickup can talk to itself', () => {
  ...
  it('can send message', async () => {
    ...
    var receivedMsg: Client.PlayerItemPickupPayload | undefined = undefined;
    ...
    var publishMessage: TestClient.PlayerItemPickupPayload = {
    ...
    const subscription = await client.subscribeToGameServerServerIdEventsPlayerPlayerIdItemItemIdPickup((err, msg, serverId, playerId, itemId) => {
        receivedError = err;
        receivedMsg = msg;
    ...
    await testClient.publishToGameServerServerIdEventsPlayerPlayerIdItemItemIdPickup(publishMessage, ServerIdToSend, PlayerIdToSend, ItemIdToSend);
