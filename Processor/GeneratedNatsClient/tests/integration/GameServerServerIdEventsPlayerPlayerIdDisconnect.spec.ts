...
describe('game/server/{serverId}/events/player/{playerId}/disconnect can talk to itself', () => {
  ...
  it('can send message', async () => {
    ...
    var receivedMsg: Client.PlayerDisconnectedPayload | undefined = undefined;
    var receivedServerId: string | undefined = undefinedvar receivedPlayerId: string | undefined = undefined
    var publishMessage: TestClient.PlayerDisconnectedPayload = {
      "disconnectTimestamp": "2019-08-24T14:15:22Z"
    };
    var ServerIdToSend: string = "string"
    var PlayerIdToSend: string = "string"
    const subscription = await client.subscribeToGameServerServerIdEventsPlayerPlayerIdDisconnect((err, msg, serverId, playerId) => {
        receivedError = err;
        receivedMsg = msg;
    ...
    await testClient.publishToGameServerServerIdEventsPlayerPlayerIdDisconnect(publishMessage, ServerIdToSend, PlayerIdToSend);
