...
export function subscribe(
  onDataCallback: (
    ...
    msg ? : AnonymousSchema_9, serverId ? : string, playerId ? : string) => void,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`game.server.${serverId}.events.player.${playerId}.hit`, (err, msg) => {
        if (err) {
